...
class Grid():
    def __init__(self, step_x, step_y, color=Color(64,64,64), width=1):
        ...
        self.surf.set_colorkey(COLOR.COLORKEY); self.surf.fill(self.surf.get_colorkey())
        if step_x > 0:
...
class Shape(pg.sprite.Sprite):
    def __init__(self, pos, size, shape):
        ...
        self.image.set_colorkey(Color(1,1,1)); self.image.fill(self.image.get_colorkey())
...
class DeleteBox(pg.sprite.Sprite):
    def __init__(self, pos, size):
        ...
        self.image = pg.Surface(size)
        self.image.set_colorkey(Color(1,1,1)); self.image.fill(self.color)
        self.rect = self.image.get_rect()
        self.mask = pg.mask.from_surface(self.image)
...
def dragNewShape(shape, color=COLOR.WHITE, step_size=0, shift_ratio=1/1):
    ...
    if not MOUSE_ACTION_STATES.SHIFT:
        ...
        temp_shape_norm_half = roundNumToNearest(np.linalg.norm(np.array([TemporaryShapeData.WIDTH, TemporaryShapeData.HEIGHT]))/2, step_size)
        TemporaryShapeData.SIZE = np.absolute(np.array([roundNumToNearest(temp_shape_norm_half*shift_ratio, step_size), temp_shape_norm_half]))
    ...
    elif TemporaryShapeData.WIDTH < 0: shape.resize_data(True, False)
    elif TemporaryShapeData.HEIGHT < 0: shape.resize_data(False, True)
    else: shape.resize_data(False, False)
def changeActionTypeWithEvent(HotBar, event):
    match event.key:
        case pg.K_1:
            GLOBALS.ACTION_TYPE = ACTION_TYPES.RECTANGLE; HotBar.switch_button()
        case pg.K_2:
            GLOBALS.ACTION_TYPE = ACTION_TYPES.CIRCLE; HotBar.switch_button()
        case pg.K_3:
            GLOBALS.ACTION_TYPE = ACTION_TYPES.TRIANGLE; HotBar.switch_button()
        case pg.K_4:
            GLOBALS.ACTION_TYPE = ACTION_TYPES.PENTAGON; HotBar.switch_button()
        case pg.K_5:
            GLOBALS.ACTION_TYPE = ACTION_TYPES.STAR; HotBar.switch_button()
        case pg.K_6:
            GLOBALS.ACTION_TYPE = ACTION_TYPES.HEXAGON; HotBar.switch_button()
        case pg.K_d:
            GLOBALS.ACTION_TYPE = ACTION_TYPES.DELETE; HotBar.switch_button()
...
class ShapeButton():
    def __init__(self, rect, action_type, bg_color, border_color, shape_color, shape_scale):
        ...
        self.bg_color = bg_color; self.surf.fill(self.bg_color)
        self.border_color = border_color
        self.shape_color = shape_color

        self.action_type = action_type; self.shape_scale = shape_scale
...
class HotBar:
    def __init__(self):
        ...
        self.button_poses = [(x + off * padding, 0) for off, x in  enumerate(range(0, button_size[0]*num_buttons, button_size[0]))]
        ...
        for pos, action_type in zip(self.button_poses[0:num_buttons-1], ACTION_TYPES.SHAPES):
            self.buttons.append(ShapeButton((pos, button_size), action_type, self.button_bg_color, self.button_bd_color, COLOR.BLUE, 0.6))
        for pos, (action_type, text) in zip([self.button_poses[num_buttons-1]], {ACTION_TYPES.DELETE: 'DEL'}.items()):
            self.buttons.append(TextButton((pos, button_size), action_type, text, 16, self.button_bg_color, self.button_bd_color))
        self.switch_button()
    def switch_button(self):
        for button in self.buttons:
            if not GLOBALS.ACTION_TYPE == button.action_type: bg_color = button.bg_color
            else: bg_color = button.bg_color - Color(50, 50, 50)
...
class ClickTextButton():
    def __init__(self, rect, text, text_height, text_color, bg_color, border_color):
        ...
        self.bg_color = Color(bg_color); self.button_surf.fill(self.bg_color)
        self.border_color = border_color
...
class PauseScreen:
    def __init__(self):
        ...
        self.button_positions = ((self.screen_rect.centerx, y + offset*10) for
                                 offset, y in enumerate(range(75, self.button_size[1]*num_buttons, self.button_size[1])))
        self.buttons = {}
        for pos, (key, text) in zip(self.button_positions, {'res':'Resume', 'clear':'Clear', 'exit':'Exit'}.items()):
            self.buttons[key] = (ClickTextButton((pos, self.button_size), text, 32, Color('BLACK'), Color(170, 170, 170), Color(120, 120, 120)))
            self.buttons[key].button_rect = self.buttons[key].button_surf.get_rect(center=self.buttons[key].button_rect.topleft)
    def draw(self, display, special_flags=0):
                ...
                if e.button == 1:
                    if not paused:
                        if any(GLOBALS.ACTION_TYPE == type for type in ACTION_TYPES.SHAPES):
                            ...
                            TemporaryShapeData.CURRENT_SHAPE = Shape(TemporaryShapeData.START, TemporaryShapeData.SIZE, GLOBALS.ACTION_TYPE)
        ...
        if e.type == pg.MOUSEBUTTONUP:
            if e.button == 1:
                if not paused:
                    if GLOBALS.MOUSE_ACTION == MOUSE_ACTION_STATES.DRAGGING:
                        if any(GLOBALS.ACTION_TYPE == type for type in ACTION_TYPES.SHAPES):
                            dragNewShape(TemporaryShapeData.CURRENT_SHAPE, Color(128, 128, 128), GLOBALS.GRID_SIZE, TemporaryShapeData.SHIFT_RATIO)
                        if GLOBALS.ACTION_TYPE == ACTION_TYPES.DELETE:
                            for shape in shapes_group:
                                if shape != TemporaryShapeData.CURRENT_SHAPE:
                                    if shape.mask.overlap(TemporaryShapeData.CURRENT_SHAPE.mask, (TemporaryShapeData.CURRENT_SHAPE.rect.x - shape.rect.x, TemporaryShapeData.CURRENT_SHAPE.rect.y - shape.rect.y))\
                                            or shape.mask.overlap(Cursor.mask, (mousepos[0] - shape.rect.x, mousepos[1] - shape.rect.y)):
    ...
    if not paused:
        ...
        if GLOBALS.MOUSE_ACTION == MOUSE_ACTION_STATES.DRAGGING:
            if any(GLOBALS.ACTION_TYPE == type for type in ACTION_TYPES.SHAPES):
                ...
                dragNewShape(TemporaryShapeData.CURRENT_SHAPE, Color(219, 55, 172, a=100), GLOBALS.GRID_SIZE, TemporaryShapeData.SHIFT_RATIO)
